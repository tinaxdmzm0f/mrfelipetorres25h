 ...
 * @since 3.4.0
 */
public class ImportVersionsWizardPage extends WizardResourceImportPage {
	...
	class DebugFileSystemElement extends FileSystemElement {
		...
		/**
		 * Populates the children of the specified parent <code>FileSystemElement
		 * @param element
		 * @param folderonly
		 */
		private void populateElementChildren() {
			...
			while(iter.hasNext()) {
				...
				if(child.isFile()) {
					...
					if(extension != null && extension.equals("cfg") {
	 ...
	 * Constructor
	 */
	public ImportVersionsWizardPage() {
